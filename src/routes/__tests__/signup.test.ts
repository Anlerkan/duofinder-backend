...
describe('tests validity of email input', () => {
  ...
  it('should return a 422 if the email is not valid', async () => {
    await request(app)
      .post(SIGNUP_ROUTE)
      .send({ email: 'invalidEmail', password, username: 'anlerkan' })
      .expect(422);
    await request(app)
      .post(SIGNUP_ROUTE)
      .send({ email: '@test.com', password, username: 'anlerkan' })
      .expect(422);
  });
  ...
  it('should return 200 if the email is valid', async () => {
    await request(app)
      .post(SIGNUP_ROUTE)
      .send({ email: 'test@test.com', password, username: 'anlerkan' })
      .expect(201);
  });
});
...
describe('tests validity of password input', () => {
  ...
  it('should return 422 if the password is not provided', async () => {
    await request(app).post(SIGNUP_ROUTE).send({ email, username: 'anlerkan' }).expect(422);
  });

  it('should return 422 if the password contains less than 8 characters', async () => {
    await request(app)
      .post(SIGNUP_ROUTE)
      .send({ email, password: 'Valid12', username: 'anlerkan' })
      .expect(422);
  });
  ...
  it('should return 422 if the password does not contain one upper case letter', async () => {
    await request(app)
      .post(SIGNUP_ROUTE)
      .send({ email, password: 'valid12valid12', username: 'anlerkan' })
      .expect(422);
  });
  ...
  it('should return 422 if the password does not contain a number', async () => {
    await request(app)
      .post(SIGNUP_ROUTE)
      .send({ email, password: 'Validvalid', username: 'anlerkan' })
      .expect(422);
  });
...
describe('tests sanitization of email input', () => {
  it('should not contain uppercase letters in the domain of the email', async () => {
    ...
    const response = await request(app)
      .post(SIGNUP_ROUTE)
      .send({ email: 'test@TEST.COM', password: 'Valid123', username: 'anlerkan' })
      .expect(201);
